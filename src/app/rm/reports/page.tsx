    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-xl border bg-white p-6">
          <h2 className="mb-1 font-semibold">Bench Age Breakdown</h2>
          <p className="mb-4 text-sm text-gray-500">Days on bench distribution</p>
          {[
        ...
        <div className="rounded-xl border bg-white p-6">
          ...
          <p className="mb-4 text-sm text-gray-500">Key performance metrics</p>
          <ul className="space-y-4 text-sm">
            ...
            <li className="flex justify-between">
              <span>⚠️ Candidates with No Match</span>
              <strong className="text-red-600">{data.candidatesWithNoMatch}</strong>
            </li>
            <li className="flex justify-between">
      ...
      <div className="rounded-xl border bg-white p-6">
        ...
        <p className="mb-4 text-sm text-gray-500">
          Candidates whose notice period is expiring soon
        </p>
        {data.noticeRisk.map((c) => (
          <div
            key={c._id}
            className="mb-3 flex items-center justify-between rounded-lg border border-red-100 bg-red-50 p-4"
          >
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-red-200 font-bold text-red-800">
                {c.name
                  .split(" ")
                  .map((n) => n[0])
                  .join("")
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {c.role} · {c.benchDays}d on bench
                </p>
              </div>
            </div>
            <div className="text-right">
              <p className="font-bold text-red-600">{c.noticePeriodDays}d</p>
              <p className="text-xs text-red-500">notice left · Urgent</p>
            </div>
          </div>
        ))}
      </div>
